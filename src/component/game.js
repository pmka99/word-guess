...
import GuessInput from "./myinput"
...
function Game(){
    ...
    const start=async()=>{
      ...
      setWord(data[Math.ceil(Math.random()*672)])
    ...
    const handler5=async(e)=>{
      if(e.target.value[0]!==''){
        ...
        if((search.length==5)&&set){
          ...
          allguess.map(v=>{if(v==search){set=false}})
          if(set){
            setv1('');setv2('');setv3('');setv4('');setv5('')
            console.log(guess)
            if(word==search){
            ...
            setv1('');setv2('');setv3('');setv4('');setv5('')
            setMessage("You have already entered this word")
          }
        }
      }
    }
